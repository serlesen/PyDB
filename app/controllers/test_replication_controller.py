...
class ReplicationControllerTest(unittest.TestCase):
    ...
    def test_sync_upsert(self):
        response = self.app.post('/admin/replicate/sync', data=json.dumps({'collection': 'col', 'docs': [{'id': 3, 'first_name': 'serser'}], 'url': 'localhost:5001'}), content_type='application/json', headers={'Authorization': 'Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpYXQiOjE1NzY1OTExNTksInN1YiI6Mn0.Uf2tyw4vPxKQ95id2iOefp4GcO3UGnYXZLPyX8NoV1U'})
    ...
    def test_sync_delete(self):
        response = self.app.delete('/admin/replicate/sync', data=json.dumps({'collection': 'col', 'search_query': {'$filter': {'id': 5}}, 'url': 'localhost:5001'}), content_type='application/json', headers={'Authorization': 'Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpYXQiOjE1NzY1OTExNTksInN1YiI6Mn0.Uf2tyw4vPxKQ95id2iOefp4GcO3UGnYXZLPyX8NoV1U'})
        self.assertEqual(response.status_code, 200)
