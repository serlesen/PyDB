...
class SearchServiceTest(unittest.TestCase):
    ...
    def test_find_doc_in_file(self):
        ...
        results = self.search_service.search(CollectionMetaData('col'), search_context, None)
    ...
    def test_search_over_250000_docs(self):
        results = self.search_service.search(CollectionMetaData('big-col'), SearchContext({'$filter': {'id': 449994}}), None)
        self.assertEqual(len(results), 1)
        self.assertEqual(results[0]['id'], 449994)

    @timeout_decorator.timeout(1)
    def test_search_over_250000_docs_with_index(self):
        results = self.search_service.search(CollectionMetaData('big-col-with-index'), SearchContext({'$filter': {'id': 449994}}), None)
        self.assertEqual(len(results), 1)
        self.assertEqual(results[0]['id'], 449994)
